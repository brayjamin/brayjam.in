...
$.util = {
   ...
   sf: function (array) {
      var output = [];
      for (var key in array) if (String(Number(key)) === key) output[+key] = array[key];
      ...
      for (var key in output) array[key] = output[key];
      return output;
   }
};
...
var mplayer = {
   ...
   flow: function (direction) {
      var array = __spreadArray(
         __spreadArray(__spreadArray([], mnav.data['index'], true), mnav.data['index'], true),
         mnav.data['index'],
         true
      );
      var offset = 3;
      ...
      var _loop_1 = function (item) {
         if (direction) item.style.animationName = ''.concat(item.id, '-').concat(direction);
   ...
   refresh: function () {
      ...
      for (var _i = 0, _a = __spreadArray([], document.querySelectorAll('.list-item'), true); _i < _a.length; _i++) {
         var li = _a[_i];
         if (li.id === 'list-item-'.concat(mnav.member)) li.setAttribute('active', '');
      ...
      history.replaceState(
         null,
         null,
         ''.concat(location.origin).concat(location.pathname, '?state=').concat(mnav.state)
      );
      resizeHeaderText();
...
var loops = [
   ...
   function () {
      ...
      if (!mplayer.volume && !ui.player.duration) {
         ...
         if (mplayer.volume) ui.current.innerText = ''.concat(Math.round(ui.player.volume * 100), '%');
         else {
            ui.current.innerText = $.util.ms(ui.player.currentTime);
            ui.total.innerText = $.util.ms(ui.player.duration);
         }
         if (!mplayer.seek) ui.fill.style.width = ''.concat(rect.width / (max / current), 'px');
      }
   },
   function () {
...
$('#rail').on({
   mousedown: function (e) {
      return mpc.seek(true), loops[0](e);
   },
   mouseenter: function () {
      return ui.fill.setAttribute('class', 'hover');
   },
   mouseleave: function () {
      return ui.fill.removeAttribute('class');
   }
});
/** Cover flow triggers */
...
$('#flow-center').on({
   click: function () {
      if (mplayer.group['type'] !== 'single') return;
      mpc.play();
   }
});
/** Hover list triggers */
$('#hover-list').on({
   ...
   mouseleave: function () {
      return $('#flow-center > img').css({ filter: null });
   }
});
/** Video triggers */
$('video').on({
...
if ('mediaSession' in navigator) {
   ...
   navigator.mediaSession.setActionHandler('play', function () {
      return mpc.play(true);
   });
   navigator.mediaSession.setActionHandler('pause', function () {
      return mpc.play(false);
   });
...
window.addEventListener('resize', function () {
   return loops[1](), loops[2](), loops[3]();
});
